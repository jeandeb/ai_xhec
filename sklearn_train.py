...
def train(args=None) :
    ...
    cross_validation = cross_validate(model, x_train, y_train, cv=5, n_jobs=-1, return_estimator=False)
    score = np.mean(np.abs(cross_validation['test_score']))
    print("cross_validation", cross_validation)
    print("cross_validation score", score)

    name = "{0}_{1}_{2}_{3}_{4}_{5}_{6}_{7}_{8}_{9}_{10}".format(args.model, args.epochs, args.lr, args.ele, args.hls, args.adaptive, args.tol, args.substances, args.no_libelle, args.bs, score)
    ...
    with open('models/models_score.csv','a') as fd:
        ...
        writer.writerow([args.model, args.epochs, args.lr, args.ele, args.hls, args.adaptive, args.tol, args.substances, args.no_libelle, args.bs, name, cross_validation['test_score'], score])

    pickle.dump(cross_validation, open("./models/cross_validation_sklearn.pk", 'wb'))
